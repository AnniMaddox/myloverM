...
| `MIN_SCORE_THRESHOLD` | stable 記憶搜尋結果最低分數門檻（0=不過濾） | `0.1` |
| `WEIGHT_KEYWORD` | 搜尋評分：關鍵字命中權重 | `0.5` |
| `WEIGHT_IMPORTANCE` | 搜尋評分：重要度權重 | `0.3` |
| `WEIGHT_RECENCY` | 搜尋評分：新鮮度權重 | `0.2` |
...
- **MIN_SCORE_THRESHOLD** — stable 記憶搜尋加入最低分數門檻，過濾不相關記憶（預設 0.1，可設環境變數）
- **記憶模型原始回傳日誌** — JSON 解析失敗時印出模型原始回傳，方便 debug
- **請求日誌** — 每次收到訊息在 Railway log 印出 session / 時間 / 字數
- **管理頁時區修正** — 記憶室時間顯示改用瀏覽器時區（Asia/Taipei）格式化
